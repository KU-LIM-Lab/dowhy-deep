...
def classify_variables(df):
    ...
    # float64 타입 + 'AMT', 'CT', 'DYCT', 'SNSC', 'RATE', 'NMPR' 등으로 끝나는 int64
    ...
    # (JHNT_MBN, JHNT_CTN과 같은 ID와 HPAR/JSCD와 같은 고유 코드 변수)
    ...
    # 중복 제거 및 최종 리스트 정리
    all_selected = set([outcome_col] + categorical_cols + continuous_cols)
...
def perform_eda(df: pd.DataFrame, output_dir: Path, logger: logging.Logger) -> None:
    ...
    # 1-1. Distribution Plot (Histogram)
    try:
        n_plots = len(T_Z_cont)
        rows = int(n_plots**0.5)
        cols = (n_plots + rows - 1) // rows # 올림
    ...
    # 2-1. Outcome Probability by Category (Bar Plot: Mean Y)
    try:
        # 최대 15개 변수만 한 번에 플로팅하여 가독성 확보
        ...
        for i, col in enumerate(plot_cols):
            # 카테고리가 너무 많은 경우 (unique > 10), 상위 5개만 사용
    ...
    # ----------------------------------------------------------------------
    # 3. Correlation Analysis (연속형 변수 간의 다중공선성 및 Y와의 연관성)
    # ----------------------------------------------------------------------
    ...
    try:
        ...
        logger.info(f"\nCorrelation with {Y}:")
        logger.info(y_corr.to_markdown(numalign="left", stralign="left", floatfmt=".3f"))
