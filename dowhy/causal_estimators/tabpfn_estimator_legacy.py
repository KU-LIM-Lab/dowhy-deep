...
class TabpfnEstimator(RegressionEstimator):
    ...
    def _build_features(self, data_df: pd.DataFrame, treatment_values=None):
        """
        Builds the features for the TabPFN model.
        """
        return NotImplementedError("not yet")
    
    def _build_model(self, data: pd.DataFrame):
        """
        Builds the TabPFN model.
        Wrapper로 구현
        """
        return NotImplementedError("not yet")
    ...
    def predict_fn(self, data: pd.DataFrame, model, features):
        """
        Predicts outcomes using the fitted TabPFN model.
        classifier: predict_proba, regressor: predict 로 분기
        """
        return NotImplementedError("not yet")
